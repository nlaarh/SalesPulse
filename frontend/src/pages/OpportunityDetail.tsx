...
/* ── Print HTML builder ─────────────────────────────────────────────────── */
function buildOppPrintHtml(d: OppDetail): string {
  const fmt = (v: number | null | undefined) => v != null ? `$${v.toLocaleString()}` : '—'
  const scoreColor = d.score >= 70 ? '#065f46' : d.score >= 40 ? '#92400e' : '#991b1b'
  const scoreBg   = d.score >= 70 ? '#d1fae5' : d.score >= 40 ? '#fef3c7' : '#fee2e2'
  const histRows = d.history.slice(0, 12).map(h =>
    `<tr><td>${h.date?.slice(0,10)||'—'}</td><td>${h.stage}</td><td style="text-align:right">${fmt(h.amount)}</td><td>${h.by||'—'}</td></tr>`
  ).join('')
  const taskRows = d.tasks.slice(0,8).map(t =>
    `<tr><td>${t.subject}</td><td>${t.status}</td><td>${t.due?.slice(0,10)||'—'}</td><td>${t.owner||'—'}</td></tr>`
  ).join('')
  const fields = [
    ['Owner', d.owner], ['Account', d.account], ['Record Type', d.record_type],
    ['Close Date', d.close_date], ['Probability', `${d.probability}%`],
    ['Forecast', d.forecast_category], ['Lead Source', d.lead_source],
    ['Destination', d.destination], ['Trip ID', d.trip_id],
    ['# Travelers', d.num_traveling != null ? String(d.num_traveling) : null],
    ['Commission', d.commission != null ? fmt(d.commission) : null],
    ['Push Count', String(d.push_count)],
    ['Coverage', d.account_coverage], ['MPI', d.account_mpi != null ? String(d.account_mpi) : null],
  ].filter(([, v]) => v != null && v !== '—')
  const fieldRows = fields.map(([k, v]) =>
    `<tr><td style="color:#6b7280;width:140px">${k}</td><td style="font-weight:600">${v}</td></tr>`
  ).join('')

  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${d.name}</title>${PRINT_STYLES}</head><body>
  <div class="header-bar">
    <p>Opportunity Detail · SalesPulse · ${new Date().toLocaleDateString()}</p>
    <h1>${d.name}</h1>
    <div style="margin-top:8px;display:flex;gap:10px;flex-wrap:wrap;align-items:center">
      <span class="badge badge-blue">${d.stage}</span>
      <span style="font-size:22px;font-weight:800;color:#fff">${fmt(d.amount)}</span>
      <span class="badge" style="background:${scoreBg};color:${scoreColor}">Health: ${d.score}/100</span>
    </div>
  </div>

  <table style="margin-bottom:18px">${fieldRows}</table>

  ${histRows ? `<h2>Stage History</h2>
  <table><thead><tr><th>Date</th><th>Stage</th><th style="text-align:right">Amount</th><th>Changed By</th></tr></thead>
  <tbody>${histRows}</tbody></table>` : ''}

  ${taskRows ? `<h2>Recent Tasks / Activities</h2>
  <table><thead><tr><th>Subject</th><th>Status</th><th>Due</th><th>Owner</th></tr></thead>
  <tbody>${taskRows}</tbody></table>` : ''}

  ${d.ai_analysis ? `<h2>AI Deal Analysis</h2>
  <div style="font-size:12px;line-height:1.6;background:#f8fafc;padding:12px;border-radius:6px;white-space:pre-wrap">${d.ai_analysis.replace(/<[^>]+>/g,'')}</div>` : ''}
      ...
      <div className="animate-enter flex items-start gap-3">
        ...
        <div className="shrink-0 text-right">
          ...
          <div className="flex items-center justify-end gap-1.5 mt-2 print:hidden">
            <button onClick={() => printWindow(buildOppPrintHtml(detail))}
