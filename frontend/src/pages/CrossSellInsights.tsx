        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-muted-foreground text-xs uppercase tracking-wider">
              ...
              <th className="text-left px-4 py-3 font-medium">City</th>
              <th className="text-center px-4 py-3 font-medium">LTV</th>
              <th className="text-center px-4 py-3 font-medium">Membership</th>
              <th className="text-center px-4 py-3 font-medium">Age / Birthday</th>
              <th className="text-center px-4 py-3 font-medium">Turning 65</th>
              <th className="text-center px-4 py-3 font-medium">Priority</th>
              <th className="text-left px-4 py-3 font-medium" style={{ minWidth: 280 }}>Recommendation</th>
            </tr>
          </thead>
          <tbody>
            {pageData.map((c, i) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  {c.days_until_65 > 0 ? (
                    <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900">
                      In {c.days_until_65} days
                    </span>
                  ) : c.days_until_65 === 0 ? (
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 animate-pulse dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900">
                      Today! 🎉
                    </span>
                  ) : (
...
type Tab = 'all' | 'needs_insurance' | 'needs_travel' | 'medicare'
...
export default function CrossSellInsights() {
  ...
  const tabs: { key: Tab; label: string; count: number }[] = [
    { key: 'all', label: 'All Opportunities', count: summary.needs_insurance_count + summary.needs_travel_count + (medSummary?.total_eligible ?? 0) },
