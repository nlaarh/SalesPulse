...
/* ── Print HTML builder ─────────────────────────────────────────────────── */
function buildDashboardPrintHtml(summary: Summary, leaders: Advisor[], line: string, periodLabel: string): string {
  ...
  const kpis = [
    ['Revenue', fmt(summary.revenue, true)],
    ['Bookings', fmt(summary.bookings, true)],
    ['Win Rate', `${(summary.win_rate||0).toFixed(1)}%`],
    ['Avg Deal', fmt(summary.avg_deal_size, true)],
    ['Pipeline', fmt(summary.pipeline_value, true)],
    ['Open Deals', String(summary.pipeline_count ?? '—')],
  ]
  const kpiHtml = kpis.map(([k,v]) =>
    `<div class="kpi-box"><div class="kpi-label">${k}</div><div class="kpi-val">${v}</div></div>`
  ).join('')
  const leaderRows = leaders.slice(0,15).map((a,i) =>
    `<tr>
      <td style="color:#6b7280;width:30px">${i+1}</td>
      <td style="font-weight:600">${a.name}</td>
      <td style="text-align:right">${fmt(a.revenue,true)}</td>
      <td style="text-align:right">${fmt(a.bookings,true)}</td>
      <td style="text-align:center">${a.deals}</td>
      <td style="text-align:center">${(a.win_rate||0).toFixed(0)}%</td>
    </tr>`
  ).join('')
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${line} Sales Dashboard</title>${PRINT_STYLES}</head><body>
  <div class="header-bar" style="background:#4f46e5">
    <p>Sales Performance · SalesPulse · ${new Date().toLocaleDateString()}</p>
    <h1>${line} Division</h1>
    <p style="margin-top:4px;font-size:12px;color:#c7d2fe">${periodLabel}</p>
  </div>

  <h2>Key Metrics</h2>
  <div class="kpi-grid">${kpiHtml}</div>

  ${leaderRows ? `<h2>Advisor Leaderboard</h2>
  <table>
    <thead><tr><th>#</th><th>Advisor</th><th style="text-align:right">Revenue</th><th style="text-align:right">Bookings</th><th style="text-align:center">Deals</th><th style="text-align:center">Win%</th></tr></thead>
    ...
    <div className="space-y-3">
      ...
      <div className="animate-enter flex items-end justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { if (summary) printWindow(buildDashboardPrintHtml(summary, leaders, line, periodLabel)) }}
