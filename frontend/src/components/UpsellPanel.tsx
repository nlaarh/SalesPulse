...
export default function UpsellPanel({ accountId }: { accountId: string }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <div
        className="px-5 py-3.5 border-b border-border flex items-center justify-between cursor-pointer select-none"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {!generated && !collapsed && (
            <button onClick={e => { e.stopPropagation(); generate(false) }} disabled={loading}
          ...
          {generated && !collapsed && (
            <button onClick={e => { e.stopPropagation(); generate(true) }} disabled={loading}
              className="text-[11px] text-muted-foreground hover:text-foreground transition-colors">
              {loading ? 'Refreshing…' : 'Refresh'}
            </button>
          )}
          <ChevronDown className={cn('h-3.5 w-3.5 text-muted-foreground/50 transition-transform', collapsed && 'rotate-180')} />
        </div>
      </div>
      {!collapsed && (
