...
export function withDates(params: Record<string, unknown>, startDate?: string | null, endDate?: string | null) {
...
export async function fetchAdvisorDashboard(
  ...
  const { data } = await api.get('/api/sales/advisors/dashboard', {
    params: withDates({
      line,
      period,
      ...(yoyYear ? { yoy_year: yoyYear } : {}),
